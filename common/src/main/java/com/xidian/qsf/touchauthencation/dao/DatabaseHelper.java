...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    private static String DATABASE_NAME = "implicate_auth";

    private static String IMPLICATE_USER_DATA = "CREATE TABLE IF NOT EXISTS implicate_user_data" +
            ...
            "touch_time INTEGER"+
            ")";
    private static String USER_CLICK_DATA = "CREATE TABLE IF NOT EXISTS user_click_data(" +
            "id INTEGER PRIMARY KEY AUTOINCREMENT, uid TEXT," +
            "pointX REAL, pointY REAL, timestamp REAL, click_time INTEGER" +
            ")";

    private static String USER_INFO = "CREATE TABLE IF NOT EXISTS user_info(" +
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(IMPLICATE_USER_DATA);
        db.execSQL(USER_CLICK_DATA);
    ...
    public void delete(SQLiteDatabase db){
        db.execSQL("drop table if exists implicate_user_data");
        db.execSQL("drop table if exists user_click_data");
    ...
    public void insert(UserBehavoir userBehavoir) {
        ...
        getWritableDatabase().insert("implicate_user_data", null, cv);
        System.out.println("insert touch data success");
    }


    public void insertUserInfo(SQLiteDatabase db, User user){
        db.execSQL("delete from user_info");
        ContentValues cv = new ContentValues();
        cv.put("username", user.getUsername());
        cv.put("passwd", user.getPassword());
        System.out.println("insert success!");
        getWritableDatabase().insert("user_info", null, cv);
    }


    public User queryUser(){
        Cursor cursor = getReadableDatabase().query("user_info", new String[]{"username"}, null, null,null, null, null);
        User user = null;
    ...
    public void insertClickData(ClickData userClickData) {
        ContentValues cv=new ContentValues();

        cv.put("uid", userClickData.getUid());
        cv.put("pointX", userClickData.getPointX());
        cv.put("pointY", userClickData.getPointY());
        cv.put("timestamp", userClickData.getTimestamp());


        userClickData.setClick_time(new Date());
        cv.put("click_time", userClickData.getClick_time().getTime());


        getWritableDatabase().insert("user_click_data", null, cv);
        System.out.println("insert click data success");
    }
